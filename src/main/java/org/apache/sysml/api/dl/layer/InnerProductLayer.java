...
public class InnerProductLayer extends Layer {
	...
	public InnerProductLayer(LayerParameter param) throws DMLRuntimeException {
		super(param, "H_");
	...
	public void generateForwardDML(TabbedStringBuilder dmlScript) throws DMLRuntimeException {
		...
		dmlScript.append(assign(getTopDMLVar(), add(matmult(getBottomDMLVar(), weightVar), biasVar)));
	}
	...
	public void generateBackwardDML(TabbedStringBuilder dmlScript) throws DMLRuntimeException {
		...
		assertBottom(1);
		dmlScript.append(assign(gradientPrefix + weightVar, matmult(t(getBottomDMLVar()), deltaVar)));
		if(innerParam.hasBiasFiller()) {
			dmlScript.append(assign(gradientPrefix + biasVar, colSums(deltaVar)));
		}
		dmlScript.append(assign(getBottomLayerDeltaVar(), matmult(deltaVar, t(weightVar))));
	}
