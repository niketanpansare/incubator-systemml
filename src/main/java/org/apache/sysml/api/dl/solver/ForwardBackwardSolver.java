...
public class ForwardBackwardSolver {
	...
	public String getForwardBackwardDML(ArrayList<Layer> dmlNet, SolverParameter solver) throws DMLRuntimeException {
		...
		for(Layer layer : dmlNet) {
			if(Barista.isTrainLayer(layer.param)) {
		...
		for(Layer layer : dmlNet) {
			if(Barista.isTrainLayer(layer.param) && layer.weightVar != null) {
				tDmlScript.append("# Update weights for " + layer.getPrettyLayerName() + "\n");
...
//		dmlScript.append("\n############################## END TEST #########################\n");
	...
	private void appendTestValidationSet(StringBuilder dmlScript, 
		...
		if(!(firstLayer instanceof DataLayer)) {
			...
			oldOutputVar = firstLayer.getTopDMLVar(0);
			Barista.blobToDMLVariableMapping.get(firstLayer.param.getTop(0)).remove(0);
			Barista.blobToDMLVariableMapping.get(firstLayer.param.getTop(0)).add(0, X);
		...
		for(Layer layer : dmlNet) {
			if(layer != firstLayer && layer != lastLayer) {
				layer.updateOutputShape();
				layer.generateForwardDML(tDmlScript);
				lastOutputLayerVar = layer.getTopDMLVar();
			}
		}
		...
		Barista.blobToDMLVariableMapping.get(firstLayer.param.getTop(0)).remove(0);
		Barista.blobToDMLVariableMapping.get(firstLayer.param.getTop(0)).add(0, oldOutputVar);
