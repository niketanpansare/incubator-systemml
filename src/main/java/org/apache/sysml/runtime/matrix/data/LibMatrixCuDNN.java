...
public class LibMatrixCuDNN extends LibMatrixCUDA {
	...
	public static void reorg_nkpq(GPUContext gCtx, String instName, MatrixObject image,
			MatrixObject outputBlock, int N, int C, int H, int W, int K, int R,
			int S, int pad_h, int pad_w, int stride_h, int stride_w, int P,
			int Q, double intermediateMemoryBudget) throws DMLRuntimeException {
		...
		long t0 = GPUStatistics.DISPLAY_STATISTICS ? System.nanoTime() : 0;
		getCudaKernels(gCtx).launchKernel("reorg_nkpq",
		...
		if (GPUStatistics.DISPLAY_STATISTICS) GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_REORG_NKPQ_KERNEL, System.nanoTime() - t0);
	}
