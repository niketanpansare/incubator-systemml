...
public class DnnGPUInstruction extends GPUInstruction {
	...
	public DnnGPUInstruction(CPOperand in1, CPOperand in2, CPOperand in3, CPOperand in4, CPOperand in5, CPOperand out, String opcode, String istr, 
			double intermediateMemoryBudget) throws DMLRuntimeException {
		super(new ReorgOperator(SwapIndex.getSwapIndexFnObject()), opcode, istr);
		if( !opcode.equals("update_adagrad_x") ) {
			throw new DMLRuntimeException("Incorrect opcode: " + opcode);
		}
		_input1 = in1;
		_input2 = in2;
	...
	public static DnnGPUInstruction parseInstruction(String str) {
		...
		else if (opcode.equalsIgnoreCase("update_adagrad_x")) {
	...
	private void processAdagradUpdateInstruction(ExecutionContext ec) {
		GPUStatistics.incrementNoOfExecutedGPUInst();
		MatrixObject input = getMatrixInputForGPUInstruction(ec, _input1.getName());
		MatrixObject dX = getMatrixInputForGPUInstruction(ec, _input2.getName());
		MatrixObject cache = getMatrixInputForGPUInstruction(ec, _input3.getName());
		double lr = (int) ec.getScalarInput(_input4.getName(), _input4.getValueType(), _input4.isLiteral()).getDoubleValue();
		double eps = (int) ec.getScalarInput(_input5.getName(), _input5.getValueType(), _input5.isLiteral()).getDoubleValue();
		int rows = LibMatrixCUDA.toInt(input.getNumRows());
		int cols = LibMatrixCUDA.toInt(input.getNumColumns());
		MatrixObject out = getDenseMatrixOutputForGPUInstruction(ec, _output.getName(), rows, cols);
		...
		LibMatrixCUDA.getCudaKernels(gCtx).launchKernel("update_adagrad_x", 
				ExecutionConfig.getConfigForSimpleVectorOperations(LibMatrixCUDA.toInt(rows*cols)),
				LibMatrixCUDA.getDensePointer(gCtx, input, instName), 
				LibMatrixCUDA.getDensePointer(gCtx, dX, instName),
				LibMatrixCUDA.getDensePointer(gCtx, cache, instName),
				lr, eps, 
				LibMatrixCUDA.getDensePointer(gCtx, out, instName),
				rows*cols);
...
//	}
	...
	public void processInstruction(ExecutionContext ec) {
		...
		else if (instOpcode.equalsIgnoreCase("update_adagrad_x")) {
			processAdagradUpdateInstruction(ec);
			return;
		}
